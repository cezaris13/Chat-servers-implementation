...
int containsUserName(char *searchedString,char **userNames){
    printf("%s\n",searchedString);
...
void getUserName(int socketFd, int *userCount, char *userNames[MAX_SIZE]){
    char atsiuskVarda[MAX_SIZE] = "ATSIUSKVARDA\n\0";
    while(1){
        ...
        if (recv(socketFd, bufUserName, sizeof bufUserName, 0) < 0){
            ...
            if(!containsUserName(bufUserName, userNames)){
                userNames[socketFd] = malloc(sizeof(char)*(MAX_SIZE+1));// su maxsizes pasizaisti
                        ...
                        for(int j = 0; j <= fdmax; j++){
                            if (FD_ISSET(j, &master)){
                                ...
                                if (j != listener){
                                    ...
                                    strcat(strcat(strcat(strcat(strcat(message,"PRANESIMAS"),userNames[i]),": "),buf),"\0");
                                    printf("%d\n",strlen(message));
                                    printf("%s",message);
